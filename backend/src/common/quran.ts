/**
 * Static Quran reference data: the 114 surahs with their ayah counts, and the
 * 30 juz' used as the examination progression. Shipped with the app so the
 * recitation and exam screens never depend on an external service.
 */
...
export const SURAH_NAMES = SURAHS.map((s) => s.name);

export function findSurah(name: string): SurahInfo | undefined {
  return SURAHS.find((s) => s.name === name.trim());
}

/**
 * Number of ayat in a range, counting across surah boundaries.
 *
 * Used to score a recitation at one point per ayah without asking the teacher
 * to count them. Returns 0 for a range that runs backwards — the caller
 * validates the range separately and reports a proper error.
 */
export function countVerses(
  ...
  const from = findSurah(fromSurah);
  const to = findSurah(toSurah);
  if (!from || !to) return 0;
  ...
  // Tail of the first surah, every surah in between, then the head of the last.
  let total = Math.max(0, from.ayahs - fromAyah + 1);
  for (let n = from.number + 1; n < to.number; n += 1) {
    total += SURAHS[n - 1].ayahs;
  }
  total += Math.max(0, Math.min(toAyah, to.ayahs));
  return total;
}

/**
 * The examination programme: the 60 ahzab, and nothing else.
 *
 * Exams are requested one hizb at a time because that is the unit a circle
 * actually tests. A juz' is simply two of them, so wherever a whole juz' is
 * the more natural thing to show — a profile header, a report — it is derived
 * from the ahzab passed rather than examined separately.
 *
 * Nothing here implies an order. A student may start at hizb 1 and work
 * forwards, start at hizb 60 and work backwards, or sit them in any order at
 * all; `order` exists only so the grid renders 1 → 60 predictably.
 */
export const HIZB_SECTIONS = Array.from({ length: 60 }, (_, i) => {
  ...
  return {
    ...
    minScore: 60,
    isRequired: true,
    description: `اختبار حفظ وإتقان الحزب ${hizb} (من الجزء ${juz})`,
  };
});

export const ALL_EXAM_SECTIONS = HIZB_SECTIONS;

/** Two ahzab make a juz' — the conversion every screen that counts in ajza' uses. */
export const HIZB_PER_JUZ = 2;

/** Whole ajza' only — for a count that has to read as "3 juz' finished". */
export function juzFromHizb(passedHizb: number): number {
  return Math.floor(passedHizb / HIZB_PER_JUZ);
}

/**
 * The same figure without rounding: 5 ahzab is two and a half ajza'.
 *
 * The memorization total on a student's record uses this one, because rounding
 * an odd number of ahzab down loses half a juz' of real work — a student who
 * passed 5 ahzab showed 2, and one who passed a single hizb showed nothing at
 * all.
 */
export function exactJuzFromHizb(passedHizb: number): number {
  return Math.round((passedHizb / HIZB_PER_JUZ) * 100) / 100;
}

/**
 * Names every hizb an exam covers, not just the first one.
 *
 * An exam may be sat on a run of ahzab — the request picks them contiguously —
 * so the primary section alone is a lie everywhere it is shown: the examiner's
 * grading form, the committee's schedule, and the notification a parent reads
 * all said "الحزب 1" for an exam that ran from 1 to 5.
 *
 * A contiguous run reads as a range, anything else as a list.
 */
export function describeSections(sections: { name: string; order: number }[]): string {
  ...
  if (contiguous) {
    // "الحزب 1" -> "1", so the range reads "الأحزاب 1 إلى 5".
