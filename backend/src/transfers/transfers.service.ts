...
export class TransfersService {
  ...
  async requestStudentTransfer(actor: AuthUser, dto: CreateStudentTransferDto) {
    ...
    if (student.circleId === dto.toCircleId) {
    ...
    await this.notifyDecisionMakers(
      'طلب نقل طالب جديد',
      `طلب نقل الطالب ${student.fullName} إلى حلقة "${target.name}"`,
    ...
    await this.activity.log({
      ...
      summary: `طلب نقل الطالب ${student.fullName} إلى حلقة ${target.name}`,
  ...
  async approve(actor: AuthUser, id: string, dto: DecideTransferDto) {
    ...
    switch (request.kind) {
      case TransferKind.STUDENT_TRANSFER:
        await this.applyStudentTransfer(request);
        break;
