...
export function Avatar({
  ...
  className?: string;
  online?: boolean;
  /** Opens the picture full-screen on click. Off inside rows that are already buttons. */
  preview?: boolean;
}) {
  // A broken or removed file falls back to the initials rather than leaving the
  // browser's torn-image placeholder in a circle.
    ...
    <span className={cx('relative inline-block shrink-0', className)} style={{ width: size, height: size }}>
      {showImage ? (
        <img
          ...
          onClick={preview ? () => setOpen(true) : undefined}
          className={cx(
            // `block` and the explicit dimensions keep a wide or tall source
            // from stretching the circle inside a flex row.
            'block h-full w-full rounded-full bg-slate-100 object-cover ring-1 ring-slate-200/70',
 ...
 *
 * Rendered through a portal so it escapes whatever card, table cell or
 * overflow-hidden row the thumbnail sits in — otherwise "click to enlarge"
 * enlarges into a clipped box.
 */
  ...
  return createPortal(
    ...
    >
      <button
        ...
        onClick={onClose}
        aria-label="إغلاق"
        className="absolute left-4 top-4 rounded-xl bg-white/10 p-2 text-white transition hover:bg-white/20"
      >
        <IconX size={20} />
      </button>
      <img
...
/**
 * A picture that is not a person: the school logo, a course cover. Keeps the
 * source's own proportions inside a fixed height instead of cropping it to a
 * square, and enlarges on click.
 */
  ...
  return (
    <>
      <img
        ...
        alt={alt ?? ''}
        onClick={preview ? () => setOpen(true) : undefined}
        className={cx('max-h-full max-w-full object-contain', preview && 'cursor-zoom-in', imgClassName, className)}
      />
...
export function ProgressBar({
  ...
  // Not rounded to whole percent: memorization is counted in ahzab, so a total
  // like 2.5 of 30 is ordinary, and rounding it away made a real half-juz' of
  // progress render as a flat zero.
  const raw = Math.min(100, Math.max(0, (value / (max || 1)) * 100));
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
        <div className={cx('h-full rounded-full transition-all', tones[tone])} style={{ width: `${raw}%` }} />
      </div>
