...
import { isExternalLink, timeAgo } from '@/lib/format';
...
export default function NotificationsPage() {
  ...
  const role = useAuthStore((x) => x.user?.role);
  ...
  /**
   * Opening a notification never dead-ends.
   *
   * Notifications are written by whichever service raised them, and one of them
   * pointed a guardian at a staff-only page — a tap on their own son's exam
   * result landed on «لا تملك صلاحية الوصول». The service that did it is fixed,
   * but the rule belongs here too: mark it read, and only navigate somewhere
   * this role can actually go.
   */
  const open = (notification: AppNotification) => {
    if (!notification.isRead) markRead.mutate(notification.id);
    if (!notification.link || !role) return;
    if (isExternalLink(notification.link)) {
      window.open(notification.link, '_blank', 'noopener,noreferrer');
      return;
    }
    if (canOpenPath(role, notification.link)) navigate(notification.link);
  };

  /** Whether the row should look clickable at all. */
  const reachable = (notification: AppNotification) =>
    !!notification.link &&
    (isExternalLink(notification.link) || !role || canOpenPath(role, notification.link));
                ...
                >
                  ...
                  <button
                    ...
                    className={cx(
                      ...
                      reachable(notification) ? 'cursor-pointer' : 'cursor-default',
                    )}
                  >
