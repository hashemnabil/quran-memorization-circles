...
import type { Exam, ExamRequest, ExamSection, ExamSectionKind, PaginatedResponse } from '@/types';
...
function RequestsTab({ isCommittee }: { isCommittee: boolean }) {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['exams', 'requests', { status, page, debounced }],
    queryFn: async () =>
      (
        await api.get<PaginatedResponse<ExamRequest>>('/exams/requests', {
          params: { page, limit: 20, status: status || undefined, search: debounced || undefined },
        })
      ).data,
  });
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  const ok = await confirm({
                                    title: 'رفض الطلب',
                                    message: `سيتم رفض طلب اختبار "${req.student.fullName}" في ${describeExamSections(req.section, req.sections)}.`,
                                    confirmLabel: 'رفض',
                                  });
  ...
  return (
    ...
    >
      <div className="mb-4 rounded-xl bg-slate-50 px-4 py-3 text-xs text-slate-600">
        ...
        <p className="mt-1">
          المقرر: <span className="font-bold text-slate-800">{describeExamSections(request.section, request.sections)}</span> — درجة النجاح{' '}
...
function ExamsTab({ status, isCommittee }: { status: string; isCommittee: boolean }) {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['exams', 'list', { status, page, debounced }],
    queryFn: async () =>
      (
        await api.get<PaginatedResponse<Exam>>('/exams', {
          params: { status, page, limit: 20, search: debounced || undefined },
        })
      ).data,
  });
    ...
    <>
      <Card className="mb-5">
        <SearchInput
          ...
          placeholder="بحث باسم الطالب أو رقمه (ST-0004) أو اسم الحزب أو الممتحن..."
        />
      </Card>
      ...
      <Card padded={false}>
        {isLoading ? (
          ...
          <EmptyState
            ...
            message={debounced ? `لا يوجد اختبار يطابق «${debounced}».` : undefined}
                ...
                <tbody>
                  {data.data.map((exam) => (
                    <tr key={exam.id}>
                      ...
                      <td>
                        <SectionBadges primary={exam.section} sections={exam.sections} />
                      </td>
  ...
  return (
    ...
    >
      <div className="mb-4 rounded-xl bg-slate-50 px-4 py-3 text-xs text-slate-600">
        ...
        <p className="mt-1">
          المقرر: <span className="font-bold text-slate-800">{describeExamSections(exam.section, exam.sections)}</span> — درجة النجاح{' '}
            ...
            {data?.map((section) => (
              <tr key={section.id}>
                ...
                <td>
                  ...
                  >
                    {EXAM_SECTION_KIND_LABELS[section.kind]}
                  </Badge>
                </td>
...
/**
 * An exam may cover several juz'/ahzab at once. The first two are shown in
 * full and the rest collapse into a count, so a request for ten sections does
 * not blow the column apart.
 */
function SectionBadges({
  primary,
  sections,
}: {
  primary: { id: string; name: string; kind?: ExamSectionKind };
  sections?: { section: { id: string; name: string; kind: ExamSectionKind } }[];
}) {
  const all = sections?.length ? sections.map((s) => s.section) : [primary];
  ...
  return (
    <div className="flex flex-wrap items-center gap-1">
      {shown.map((s) => (
        <Badge
          key={s.id}
          className={s.kind === 'HIZB' ? 'bg-teal-100 text-teal-800' : 'bg-gold-100 text-gold-800'}
        >
          {s.name}
        </Badge>
      ))}
      {extra > 0 && (
        <Badge className="bg-slate-100 text-slate-600" >
          <span className="numeric">+{extra}</span>
        </Badge>
      )}
    </div>
  );
