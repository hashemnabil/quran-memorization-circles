...
import { formatDateShort, formatDateTime, formatParts, timeAgo } from '@/lib/format';
      ...
      {child && (
        <div className="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            tone={child.attendance.rate >= 80 ? undefined : child.attendance.rate >= 60 ? 'amber' : 'red'}
          />
          {/* Only the attendance rate is tinted, and only when it is low: a
              count of absences is a fact, not an alarm. */}
          <StatCard label="مرات الغياب" value={child.attendance.absent} icon={<IconClipboard size={22} />} />
          <StatCard label="الأجزاء المحفوظة" value={formatParts(child.memorizedParts)} icon={<IconBook size={22} />} hint="من 30 جزءاً" />
          <StatCard label="الاختبارات المجتازة" value={passed} icon={<IconAward size={22} />} />
        </div>
      )}
        ...
        <Card className="mb-5" title="المستوى والتقييم">
          <div className="mb-4">
            <div className="mb-1.5 flex justify-between text-xs">
              ...
              <span className="numeric font-bold text-slate-700">{formatParts(child.memorizedParts)} / 30</span>
            </div>
