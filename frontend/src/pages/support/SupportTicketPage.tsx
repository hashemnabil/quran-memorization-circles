            ...
            {ticket.messages?.map((message) => {
              ...
              return (
                <div key={message.id} className={cx('flex gap-3', mine && 'flex-row-reverse')}>
                  <Avatar name={message.sender.fullName} src={message.sender.avatarUrl} size={36} preview />
        ...
        <div className="space-y-5">
          <Card title="تفاصيل التذكرة">
            <dl className="space-y-2.5 text-xs">
              <Row label="مقدّم الطلب" value={ticketRequesterName(ticket)} />
              {/* A ticket from the login screen has contact details instead of
                  an account — they are the only way to reach the requester. */}
              {ticket.createdBy ? (
                <Row label="الدور" value={ROLE_LABELS[ticket.createdBy.role]} />
              ) : (
                <>
                  <Row label="المصدر" value="طلب من صفحة الدخول" />
                  <Row label="جوال التواصل" value={ticket.contactPhone} />
                  <Row label="بريد التواصل" value={ticket.contactEmail} />
                </>
              )}
