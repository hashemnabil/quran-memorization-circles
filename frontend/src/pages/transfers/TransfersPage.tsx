...
function DecisionModal({
  ...
  // Only a student transfer needs a destination: the requester deliberately did
  // not choose one, so placing the student is part of the approval.
  const needsCircle = approve && request.kind === 'STUDENT_TRANSFER';
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.patch(`/transfers/${request.id}/${approve ? 'approve' : 'reject'}`, {
        decisionNote: note || undefined,
        ...(needsCircle ? { toCircleId } : {}),
      }),
    onSuccess: () => {
  ...
  const submit = () => {
    if (needsCircle && !toCircleId) {
      setError('يجب تحديد الحلقة التي سيُنقل إليها الطالب');
      return;
    }
    setError('');
    mutation.mutate();
  ...
  return (
    <Modal
      ...
      footer={
        <>
          ...
          <Button variant={approve ? 'success' : 'danger'} onClick={submit} loading={mutation.isPending}>
    ...
    >
      ...
      >
        {approve
          ? needsCircle
            ? 'حدّد الحلقة المناسبة للطالب، وسيُنفَّذ النقل مباشرة بعد الاعتماد مع إشعار الأطراف المعنية.'
      ...
      <dl className="mb-4 space-y-1.5 rounded-xl bg-slate-50 px-4 py-3 text-xs">
        ...
        <div className="flex justify-between">
          ...
          <dd className="font-bold text-slate-700">
            {request.toCircle?.name ?? (needsCircle ? 'تحددها الإدارة' : '—')}
          </dd>
        </div>
      ...
      {needsCircle && (
        <Select
          ...
          value={toCircleId}
          onChange={(e) => {
            setToCircleId(e.target.value);
            setError('');
          }}
          className="mb-4"
          hint="تختارها الإدارة عند الموافقة وليس مقدّم الطلب"
        >
          <option value="">— اختر الحلقة —</option>
          {(circles ?? [])
            .filter((c) => c.id !== request.fromCircle?.id)
            .map((c) => (
